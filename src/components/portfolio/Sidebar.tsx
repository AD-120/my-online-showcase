...
const Sidebar = ({
  ...
  return <>
      {/* Mobile menu button */}
      <button onClick={() => setIsOpen(!isOpen)} className="fixed top-4 left-4 z-[60] md:hidden p-3 bg-card border-2 border-primary neo-shadow-black px-[4px] py-[4px]" aria-label="Toggle menu">
        {isOpen ? <X size={24} /> : <Menu size={24} />}
      </button>

      {/* Mobile overlay */}
      {isOpen && <div className="fixed inset-0 bg-primary/50 z-40 md:hidden" onClick={() => setIsOpen(false)} />}
      ...
      <aside className={`
        ...
        <div className="flex flex-col h-full p-8">
          {/* Logo / Name */}
          <motion.div className="mb-12 cursor-pointer" onClick={onHomeClick} whileHover={{
          scale: 1.02
        }} whileTap={{
          scale: 0.98
        }}>
            ...
            <ul className="space-y-1">
              <li>
                ...
                setIsOpen(false);
                    ...
                    ${activeCategory === 'All' ? 'border-primary bg-secondary text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground hover:border-muted hover:bg-secondary'}
                  `}>
                  All Work
                </button>
              </li>
              {CATEGORIES.map(category => <li key={category}>
                ...
                setIsOpen(false);
                      ...
                      w-full text-left py-3 px-4 text-sm font-bold uppercase tracking-widest
                      transition-all duration-200 border-l-4
                      ${activeCategory === category ? getActiveCategoryStyle(category) : `border-transparent text-muted-foreground ${getCategoryColor(category)}`}
                    `}>
                    {category}
                  </button>
                </li>)}
